...
-- CONCAT, CONCAT_WS = adding two or more column together. 
-- CONCAT_WS = this is a word sperator. 
...
-- LIKE (SPECIFING THE CHAR)

SELECT * FROM customer_tbl WHERE cust_name LIKE 'r%';-- STARTING WITH R

SELECT * FROM customer_tbl WHERE cust_name LIKE 'r%l'; -- STARTING WITH R AND END WITH L

SELECT * FROM customer_tbl WHERE city LIKE '%e'; -- ENDING WITH E

SELECT * FROM customer_tbl WHERE cust_name LIKE 'ra%';

SELECT * FROM customer_tbl WHERE city LIKE '%nn%';-- MIDDLE WITH NN

SELECT * FROM customer_tbl WHERE cust_name LIKE 'M%' OR cust_name LIKE 'A%'; -- BOTH ARE TAKEN INTO CONSIDERATION
...
START TRANSACTION; -- FOR GETTING CONTROL FROM MY SIDE 
SELECT * FROM transaction_tbl;

UPDATE transaction_tbl SET amount=150000 WHERE trans_id=8;
SELECT * FROM transaction_tbl;
COMMIT; -- CHANGES THAT ARE MADE ARE NOW PERMANENT. 
...
START TRANSACTION;
-- SAVE POINT HELPS IN PARTIAL ROLLBACK AND COMMIT
SAVEPOINT sp1;
